...
def run_task(spec, include_traces=False):
    episodes = generate_cyber_episodes(spec.n_episodes, seed_offset=spec.seed_offset)
    results = []
    for index, cfg in enumerate(episodes):
        ...
        results.append(
            EpisodeResult(
                ...
                metadata={"queries_used": outcome.get("queries_used", env.turn), "group_size": len(cfg.affected_group)},
            )
        )
    return results
